...
class QubicFullBandSystematic(QubicPolyAcquisition):
    ...
    def __init__(self, d, Nsub, Nrec=1, comp=[], kind='Two'):
        ...
        #self.ndets, self.nsamples, _ = self.Proj[0].shapeout
    ...
    def get_components_operator(self, beta, nu):
        
        return ReshapeOperator((1, 12*self.scene.nside**2, 3), (12*self.scene.nside**2, 3)) * \
                get_mixing_operator(beta, nu, self.comp, self.scene.nside, Amm=None, active=False)
    '''
    def get_distribution_operator(self):
        return MPIDistributionIdentityOperator(self.d['comm'])
    def get_unit_conversion_operator(self, nu):
        return self.scene.get_unit_conversion_operator(nu)
    def get_aperture_integration_operator(self, ins):
        return ins.get_aperture_integration_operator()
    def get_filter_operator(self, ins):
        return ins.get_filter_operator()
    ...
    def get_polarizer_operator(self):
        ...
        """
        return qubic.QubicAcquisition(self.multiinstrument[0], self.sampling, self.scene, self.d).get_polarizer_operator()
        #nd = self.Proj[0].shapeout[0]
        #nt = len(self.sampling)
        #print(nd, nt)
        #print(self.multiinstrument[0].detector.quadrant.shape)
        #grid = (self.multiinstrument[0].detector.quadrant - 1) // 4

        #z = np.zeros(self.ndets)
        #data = np.array([z + 0.5, 0.5 - grid, z]).T[:, None, None, :]
        #return ReshapeOperator((self.ndets, self.nsamples, 1), (self.ndets, self.nsamples)) * \
        #       DenseBlockDiagonalOperator(data, shapein=(self.ndets, self.nsamples, 3))
    def get_transmission_operator(self):
        """
        Return the operator that multiplies by the cumulative instrumental
        transmission.
        """
        ins = self.multiinstrument[0]
        return DiagonalOperator(
            np.product(ins.optics.components['transmission']) *
            ins.detector.efficiency, broadcast='rightward')
    def get_detector_integration_operator(self, ins):
        """
        Integrate flux density in detector solid angles.
        Convert W / sr into W.

        """
        return ins.get_detector_integration_operator()
    def get_detector_response_operator(self, tau=None):
        ...
        """
        if tau is None:
            tau = self.multiinstrument[0].detector.tau
        sampling_period = self.sampling.period
        shapein = self.Proj[0].shapeout[0], len(self.sampling)
    ...
    def sum_over_band(self, h):
        ...
        ### Components Map-Making
            ...
            if self.kind == 'wide':
                return AdditionOperator(h)
            else:
                return BlockColumnOperator([AdditionOperator(h[:int(self.Nsub)]), AdditionOperator(h[int(self.Nsub):int(2*self.Nsub)])], axisout=0)
    def get_operator(self, beta=None, angle_hwp=None, fwhm=None):
        ...
        for isub in range(self.Nsub*2):
            ...
            with rule_manager(inplace=True):
                hi = CompositionOperator([
                            response, trans_atm, trans, integ, polarizer, (hwp * projection),
                            filter, aperture, temp, distribution, convolution, Acomp])
            
            self.operator.append(hi)
            
        H = self.sum_over_band(self.operator)
    ...
    def get_PySM_maps(self, config, r=0, Alens=1):
        ...
        for k, kconf in enumerate(config.keys()):
            if kconf == 'cmb':
                ...
                nu0 = 150#self.comp[k].__dict__['_fixed_params']['nu0']
...
class JointAcquisitionComponentsMapMaking:
    ...
    def __init__(self, d, kind, comp, Nsub, nus_external, nintegr):
        ...
        self.nintegr = nintegr
        #self.qubic = qubic
        self.qubic = QubicFullBandSystematic(self.d, comp=self.comp, Nsub=self.Nsub, Nrec=1, kind=self.kind)
        self.scene = self.qubic.scene
        self.external = OtherDataParametric(self.nus_external, self.scene.nside, self.comp, self.nintegr)

    def get_operator(self, beta):

        Hq = self.qubic.get_operator(beta=beta)
        Rq = ReshapeOperator(Hq.shapeout, (Hq.shapeout[0]*Hq.shapeout[1]))
        He = self.external.get_operator(beta=beta, convolution=False)
